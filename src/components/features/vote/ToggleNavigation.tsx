...
export default function ToggleNavigation({
  ...
  const handleSwitch = (i: number) => {
    ...
    router.push(routes[i] ?? '/vote/ongoing');
  };
  ...
  return (
    <div
      className={tw(
        'relative w-full max-w-xs sm:max-w-sm md:max-w-md rounded-full bg-[#f5f5f5] shadow-[inset_0_0_10px_rgba(0,0,0,0.25)] p-1.5 sm:p-2 dark:bg-background-black2 dark:border dark:border-primary-gray2',
        className,
      )}
    >
      <div className="relative flex items-center justify-between rounded-full overflow-hidden">
        <motion.div
          ...
          initial={false}
          animate={{ left: `${sectionWidth * selected}%`, width: `${sectionWidth}%` }}
        ...
        {options.map((option, index) => (
          <button
            ...
            className={tw(
              'relative z-10 flex-1 py-1.5 sm:py-2 text-sm sm:text-base md:text-lg font-medium transition-colors duration-200',
              selected === index
                ? 'text-primary-white'
